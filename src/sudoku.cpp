...
void generatePuzzle(int full[N][N], int puzzle[N][N], int toRemove) {
    ...
    // 保存最终生成的数独谜题CNF
    SATList* finalCNF = nullptr;
    sudokuToCNF(puzzle, finalCNF);
    
    time_t now = time(0);
    tm* ltm = localtime(&now);
    char filename[256];
    snprintf(filename, sizeof(filename), "sudoku_puzzle_%04d%02d%02d_%02d%02d%02d.cnf", 
            1900 + ltm->tm_year, 1 + ltm->tm_mon, ltm->tm_mday,
            ltm->tm_hour, ltm->tm_min, ltm->tm_sec);
    
    saveSudokuCNF(finalCNF, filename);
    destroyClause(finalCNF);
}
...
bool saveSudokuCNF(SATList* cnf, const char* filename) {
    ...
    // 写入DIMACS格式头部
    fprintf(fp, "c Sudoku CNF generated by SAT solver\n");
    fprintf(fp, "c Variables: 1-729 (row*81 + col*9 + num)\n");
    fprintf(fp, "p cnf 729 %d\n", actualClauseCount);
    
    // 写入所有子句
    current = cnf;
    while (current) {
        SATNode* node = current->head;
        while (node) {
    ...
    fclose(fp);
    cout << "CNF已保存到文件: " << filename << endl;
    cout << "变量数: 729, 子句数: " << actualClauseCount << endl;
    return true;
}
